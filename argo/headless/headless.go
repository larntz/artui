...
// https://github.com/argoproj/argo-cd/blob/v2.5.4/cmd/argocd/commands/headless/headless.go
//
...
func StartLocalServer(ctx context.Context, clientOpts *apiclient.ClientOptions, ctxStr string, port *int, address *string) error {
	...
	namespace := ""
	// check if Namespace is already defined before we look at the current kube context
	// If namespace was defined in the ARTUI config we want to use that to be able to find `argocd-cm`
	if clientOpts.KubeOverrides.Context.Namespace != "" {
		namespace = clientOpts.KubeOverrides.Context.Namespace
	} else {
